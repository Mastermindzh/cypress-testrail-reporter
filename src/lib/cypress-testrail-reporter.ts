...
export class CypressTestRailReporter extends reporters.Spec {
  ...
  private reporterOptions: TestRailOptions;
  private suiteId: any = [];
  ...
  constructor(runner: any, options: any) {
    ...
    if (this.reporterOptions.singleRun === undefined) {
      this.reporterOptions.singleRun = false;
    }
    ...
    if (this.suiteId && this.suiteId.toString().length) {
      runner.on("start", () => {
        ...
        if (this.reporterOptions.singleRun && !globalRunId) {
      ...
      runner.on("retry", (test) => {
        this.submitResults(
          Status.Retest,
          test,
          "Cypress retry logic has been triggered!"
        );
      });
